...
export const t = {
  ...
  oobe: {
    ...
    existingOption: '读取现有数据',
    existingOptionDesc: '在以下位置检测到之前保存的数据，选中后原样读取，不会移动或复制文件。',
    customOption: '选择其他文件夹…',
    customHint: '定位到一个空文件夹新建数据，或选择已有的萃序数据文件夹直接读取。',
    customPicked: '已选择',
    customHasData: '将读取其中的现有数据',
    notWritable: '该文件夹不可写，请换一个位置。',
    notEmpty: '所选文件夹不是空的，也不是有效的萃序数据文件夹，请换一个位置。',
    pickFolder: '选择文件夹',
    start: '开始使用',
    starting: '正在准备…',
  },
  ...
  exportDialog: {
    ...
    fmtXlsx: 'Excel 表格',
    fmtXlsxDesc: '一行一条的 .xlsx，图片嵌入单元格',
    fmtPdf: 'PDF',
    ...
    folderImageLayout: '图片文件夹',
    folderImageRow: '按条目分行（每个条目一个文件夹，图片随条目存放）',
    folderImageColumn: '按字段分列（每个字段一个文件夹，收集该列全部图片）',
  ...
  settings: {
    ...
    openFailed: '无法打开该文件夹，请检查它是否仍然存在',
    openAbout: '关于与更新',
    about: '关于',
